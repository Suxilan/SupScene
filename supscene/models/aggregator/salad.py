...
class SALAD(nn.Module):
    ...
    def forward(self, x, return_maps=False):
        ...
        repeat_p = p.unsqueeze(1).repeat(1, self.cluster_dim, 1, 1)
        repeat_f = f.unsqueeze(2).repeat(1, 1, self.num_clusters, 1)

        f = torch.cat([
            nn.functional.normalize(t, p=2, dim=-1),
            nn.functional.normalize((repeat_f * repeat_p).sum(dim=-1), p=2, dim=1).flatten(1)
        ], dim=-1)
        
        if return_maps:
            assignment_maps = p.view(B, self.num_clusters, H, W)
            return nn.functional.normalize(f, p=2, dim=-1), assignment_maps, None, None, None
    ...
    @property
    def output_dim(self):
        return self.cluster_dim * self.num_clusters + self.token_dim